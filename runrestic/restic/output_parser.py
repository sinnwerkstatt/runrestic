...
def parse_backup(output: str) -> dict:
    files_new, files_changed, files_unmodified = re.findall(
        "Files:\s+([0-9]+) new,\s+([0-9]+) changed,\s+([0-9]+) unmodified", output
    )[0]
    dirs_new, dirs_changed, dirs_unmodified = re.findall(
        "Dirs:\s+([0-9]+) new,\s+([0-9]+) changed,\s+([0-9]+) unmodified", output
    )[0]
    added_to_the_repo = re.findall(
        "Added to the repo:\s+(-?[0-9.]+ [a-zA-Z]*B)", output
    )[0]
    processed_files, processed_size, processed_time = re.findall(
        "processed ([0-9]+) files,\s+(-?[0-9.]+ [a-zA-Z]*B) in ([0-9]+:+[0-9]+)", output
    )[0]
...
def parse_prune(output: str) -> dict:
    containing_packs_before, containing_blobs_before, containing_size_before = re.findall(
        "repository contains ([0-9]+) packs \(([0-9]+) blobs\) with (-?[0-9.]+ ?[a-zA-Z]*B)",
        output,
    )[
        0
    ]
    duplicate_blobs, duplicate_size = re.findall(
        "([0-9]+) duplicate blobs, (-?[0-9.]+ ?[a-zA-Z]*B) duplicate", output
    )[0]
    in_use_blobs, _, removed_blobs = re.findall(
        "found ([0-9]+) of ([0-9]+) data blobs still in use, removing ([0-9]+) blobs",
        output,
    )[0]
    invalid_files = re.findall("will remove ([0-9]+) invalid files", output)[0]
    deleted_packs, rewritten_packs, size_freed = re.findall(
        "will delete ([0-9]+) packs and rewrite ([0-9]+) packs, this frees (-?[0-9.]+ ?[a-zA-Z]*B)",
        output,
    )[0]
