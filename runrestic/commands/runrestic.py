...
from runrestic.config import log, signals, validate
from runrestic.config.collect import collect_config_filenames, get_default_config_paths
...
def parse_configuration(config_filename):
    logger.info(
        "Parsing configuration file: {config_filename}".format(
    ...
    with open(config_filename) as file:
        try:
            ...
            logger.warning(
                "Problem parsing {config_filename}: {e}\n".format(
                    config_filename=config_filename, e=e
                )
            )
            return
...
def run_configuration(config, args):
    ...
    log_metrics = (
        config.get("metrics") and not args.dry_run and not args.action == ["init"]
    )
    ...
    if any(rcs):
        logger.error(
            "There were problems in this run. Add `-l debug` to get a more comprehensive output"
        )
...
def main():
    ...
    try:
        ...
        if len(config_filenames) == 0:
            raise ValueError(
                "Error: No configuration files found in {}".format(
                    get_default_config_paths()
                )
            )

        configs = [
            parse_configuration(config_filename) for config_filename in config_filenames
        ]
